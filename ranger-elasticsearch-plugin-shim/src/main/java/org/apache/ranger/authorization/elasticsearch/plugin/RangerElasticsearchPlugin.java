...
public class RangerElasticsearchPlugin extends Plugin implements ActionPlugin {
    ...
    private static final Logger LOG = Loggers.getLogger(RangerElasticsearchPlugin.class, null);
                ...
                if (classLoader instanceof URLClassLoader) {
                    AccessController.doPrivileged((PrivilegedAction<Object>) () -> {
                        try {
                            ...
                            LOG.info("Success to add ranger elasticsearch plugin config directory [{}] to classpath.", configFilePath);
                        } catch (Exception e) {
                            LOG.error("Failed to add ranger elasticsearch plugin config directory [{}] to classpath.", configFilePath);
                        }
                        return null;
                    });
                }
